...
public interface DataFactoryFunctions {
    ...
    static <T> Data<T> replaceNameAndMessage(Data<T> data, String nameof, String message) {
        return getWith(data.OBJECT(), data.STATUS(), nameof, message, data.EXCEPTION(), data.EXCEPTION_MESSAGE());
    }
    ...
    static <T> Data<T> replaceMethodMessageData(Data<T> data, MethodMessageData message) {
        return getWith(data.OBJECT(), data.STATUS(), message, data.EXCEPTION(), data.EXCEPTION_MESSAGE());
    }

    static <T> Data<T> replaceObject(Data<?> data, T object) {
        return getWith(object, data.STATUS(), data.MESSAGE(), data.EXCEPTION(), data.EXCEPTION_MESSAGE());
    }

    static <T> Data<T> replaceStatus(Data<T> data, boolean status) {
        return getWith(data.OBJECT(), status, data.MESSAGE(), data.EXCEPTION(), data.EXCEPTION_MESSAGE());
    }

    static <T> Data<T> replaceMessage(Data<T> data, String message) {
        return getWith(data.OBJECT(), data.STATUS(), data.MESSAGE().NAMEOF(), data.MESSAGE().MESSAGE(), data.EXCEPTION(), data.EXCEPTION_MESSAGE());
    }

    static <T> Data<T> replaceObjectAndMessage(Data<?> data, T object, String message) {
        return getWith(object, data.STATUS(), data.MESSAGE().NAMEOF(), message, data.EXCEPTION(), data.EXCEPTION_MESSAGE());
    }

    static <T> Data<T> replaceObjectStatusAndName(Data<?> data, T object, boolean status, String nameof) {
        return getWith(object, status, nameof, data.MESSAGE().MESSAGE(), data.EXCEPTION());
    }

    static <T> Data<T> replaceStatusAndMessage(Data<T> data, boolean status, String message) {
        return getWith(data.OBJECT(), status, data.MESSAGE().NAMEOF(), message, data.EXCEPTION(), data.EXCEPTION_MESSAGE());
    }

    static <T> Data<T> replaceName(Data<T> data, String nameof) {
        return getWith(data.OBJECT(), data.STATUS(), nameof, data.MESSAGE().MESSAGE(), data.EXCEPTION(), data.EXCEPTION_MESSAGE());
    }

    static <T> Data<T> prependMessage(Data<T> data, String message) {
        return getWith(data.OBJECT(), data.STATUS(), message + data.MESSAGE(), data.EXCEPTION(), data.EXCEPTION_MESSAGE());
    }

    static <T> Data<T> prependMessage(Data<T> data, String nameof, String message) {
        return getWith(data.OBJECT(), data.STATUS(), nameof, message + data.MESSAGE().MESSAGE(), data.EXCEPTION(), data.EXCEPTION_MESSAGE());
    }
    ...
    static <T> Data<T> appendMessage(Data<T> data, String message) {
        return getWith(data.OBJECT(), data.STATUS(), data.MESSAGE() + message, data.EXCEPTION(), data.EXCEPTION_MESSAGE());
    }

    static <T> Data<T> appendMessage(Data<T> data, String nameof, String message) {
        return getWith(data.OBJECT(), data.STATUS(), nameof, data.MESSAGE().MESSAGE() + message, data.EXCEPTION(), data.EXCEPTION_MESSAGE());
    }

    static Data<Boolean> getBoolean(boolean status, String nameof, String message, Exception exception, String exceptionMessage) {
        return NullablePredicates.areNotNull(message, nameof, exception, exceptionMessage) && status ? getWith(status, status, nameof, message, exception, exceptionMessage) : CoreDataConstants.NULL_BOOLEAN;
    }
    ...
    static Data<Object[]> getArrayWithName(Object[] object, boolean status, String nameof) {
        return NullablePredicates.areNotNull(object, nameof) ? (
            getWith(object, status, nameof, "Element was found. Array of length " + object.length + " was constructed.\n")
    ...
    static Data<Object[]> getArrayWithName(Object[] object) {
        return NullablePredicates.isNotNull(object) ? (
            getWith(object, true, "getArrayWithName", "Element was found. Array of length " + object.length + " was constructed.\n")
        ) : CoreDataConstants.NULL_PARAMETER_ARRAY;
    }
}
